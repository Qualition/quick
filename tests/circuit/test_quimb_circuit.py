...
from quick.circuit import QuimbCircuit
...
class TestQuimbCircuit(Template):
    """ `tests.circuit.TestQuimbCircuit` is the tester class for `quick.circuit.QuimbCircuit` class.
    """
    @pytest.mark.parametrize("num_qubits, qubit_indices, expected", [
        [1, 0, np.eye(2**1, dtype=np.complex128)],
        [2, 1, np.eye(2**2, dtype=np.complex128)],
        [3, 2, np.eye(2**3, dtype=np.complex128)],
        [1, [0], np.eye(2**1, dtype=np.complex128)],
        [3, [0, 1], np.eye(2**3, dtype=np.complex128)],
        [3, [0, 2], np.eye(2**3, dtype=np.complex128)],
        [3, [1, 2], np.eye(2**3, dtype=np.complex128)],
        [3, [0, 1, 2], np.eye(2**3, dtype=np.complex128)]
    ])
    def test_Identity(
            self,
            num_qubits: int,
        ...
        # Define the `quick.circuit.QuimbCircuit` instance
        circuit = QuimbCircuit(num_qubits)
    ...
    def test_get_unitary(self) -> None:
        ...
        # Define the equivalent `quick.circuit.QuimbCircuit` instance, and
        # ensure they are equivalent
        unitary_circuit = QuimbCircuit(4)
