...
from qickit.circuit.gate_matrix import RZ, CX
...
class TwoQubitDecomposition(UnitaryPreparation):
    """ `qickit.synthesis.unitarypreparation.TwoQubitDecomposition` is
    ...
    def u4_to_su4(u4: NDArray[np.complex128]) -> tuple[NDArray[np.complex128], float]:
        """ Convert a 4x4 unitary matrix to a SU(4) matrix.
    ...
    def traces(target: TwoQubitWeylDecomposition) -> list[complex]:
        """ Calculate the expected traces :math:`|Tr(U \\cdot Utarget^dag)|`
        for different number of basis gates.

        Parameters
        ----------
        `target` : qickit.synthesis.gate_decompositions.two_qubit_decomposition.weyl.TwoQubitWeylDecomposition
    ...
    def trace_to_fidelity(trace: complex) -> float:
        ...
        ..math::
            `Fbar = (d + |Tr (Utarget \\cdot U^dag)|^2) / d(d+1)`
    ...
    def _decomp0(weyl_decomposition: TwoQubitWeylDecomposition) -> tuple[NDArray[np.complex128], NDArray[np.complex128]]:
        ...
        Parameters
        ----------
        `weyl_decomposition` : qickit.synthesis.gate_decompositions.two_qubit_decomposition.weyl.TwoQubitWeylDecomposition
            The target Weyl decomposition.
    ...
    def _decomp2_supercontrolled(weyl_decomposition: TwoQubitWeylDecomposition) -> tuple[
        ...
        `U0l` : NDArray[np.complex128]
            The left unitary matrix.
        """
        # Hardcoded basis when using CX as KAK basis
        q0l = np.array([
            [0.5+0.5j, 0.5-0.5j],
            [-0.5-0.5j, 0.5-0.5j]
        ], dtype=complex)
    ...
    def _decomp3_supercontrolled(weyl_decomposition: TwoQubitWeylDecomposition) -> tuple[
        ...
        u1rb: NDArray[np.complex128] = np.array([
            [-1, -1j],
            [-1j, -1]
        ], dtype=complex) * 1/np.sqrt(2)
        ...
        U1l = u1l
        U1r = u1ra.dot(RZ(-2 * float(weyl_decomposition.c)).matrix).dot(u1rb)
